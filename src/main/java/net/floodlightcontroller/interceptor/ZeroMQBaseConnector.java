...
public class ZeroMQBaseConnector implements Runnable {
	...
	public boolean SendData(byte[] data) {
		ZMsg msg = new ZMsg();
		if (data[1] == 17) {
			LOG.info("Send to >>>>>core NetIP Type==openflow message");
			}
		else {
			LOG.info("Send to >>>>>core NetIP Type:" +data[1] );
		}
			...
			if (poller.pollin(0)) {
				...
				if (coreListener != null) {
					...
					if (msg instanceof HelloMessage) {
						...
						LOG.info("NetIDEProtocolVersion:" +msg.getHeader().getNetIDEProtocolVersion());
						LOG.info("SupportedProtocols:" +((HelloMessage) msg).getSupportedProtocols());
						...
						LOG.info("Receive<<<<<<<<<<<<<<< Openflowtype:  " +((OpenFlowMessage) msg).getOfMessage().getType() );
